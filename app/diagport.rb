...
OptionParser.new do |opts|
	...
	opts.on("-h", "--hspt HOST:PORT_INDEX", "Check port of host.") do |v|
		...
		if x == nil || x.length != 4 
			pwarn "Wrong host:port format n.n.n.n:x/x:x or n.n:x/x:x or n-n:x/x:x",0
			exit
		end
		options[:host] = "192.168.#{x[1].to_i}.#{x[2].to_i}"
		q = x[3].match(/([0-9]+)\/([0-9]+):([0-9]+)/)
		if q != nil
			port = "#{q[1].to_i}/#{q[2].to_i}:#{q[3].to_i}"		
			options[:port] = port
			next
		end
		q = x[3].match(/([0-9]+)\/([0-9]+)/)
		if q != nil
			port = "#{q[1].to_i}/#{q[2].to_i}"		
			options[:port] = port
			next
		end
		q = x[3].match(/([0-9]+)/)
		...
		pwarn "Wrong port format x/x:x or x",0
		exit
	end
